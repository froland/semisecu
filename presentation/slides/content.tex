...
security in web applications. In the software engeneering world, web
...
decryp the digest depending upon the goal. If the sender wants his
message to be decodable only by a receiver, he encrypts it with the
receiver public key and the reciver decrypt it with his private key.
...
message. Sometimes, it's desirable that two identical message doesn't
...
\item When he wants a service the client ask the distribution center
  for a service ticket giving his authentication ticket as a proof of authentication.
\item The client gives thesservice ticket, which contains the service
...
techniques.
This modeling technique enable us to build the attack tree
...
interface (see \autoref{lst:parametrized-interface}). One can also use
...
a form such as \ref{CSRF-protected-form}. The hidden field is set by
...
applied to prevent an attacker to use a precomputated list of
message/hash pair to find a match. Such list are named rainbow tables.
...
conversation) and spoofing (somebody impersonating your correspondant).
...
to follow a protocol conversation to detect anaomalies. It enables it to catch SYN port
scans.
...
In this document we have reviewed various threat to web application
