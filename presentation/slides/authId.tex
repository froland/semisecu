...
\note{
\begin{block}{Identity}
The definition also applies to non-humans (systems).
\end{block}
}
\end{frame}
...
key infrastructure (PKI): you start with a certificate provided by a root CA,
usually embedded into the application or browser, and you check the other
...
\item Browsers are distributed with some preinstalled certificates.
  You can also add them manually.
\item When a website owner wants a certificate he also generates
  a public-private key pair. Then, he makes it sign by a CA.
\item The CA checks the integrity of the request and performs some
  checks on the identity of the owner. Finally, it signs the certificate.
