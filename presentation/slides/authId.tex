...
%TODO: add a schema to help identify the concepts and mark difference between each one
\begin{frame}
\begin{columns}
...
\includeimage[scale=0.2]{identity}
\end{column}
\end{columns}
...
\end{column}
\begin{column}{0.6\linewidth}
\includeimage[scale=0.3]{authentication}
\end{column}
\end{columns}
...
\end{column}
\begin{column}{0.6\linewidth}
\includeimage[scale=0.3]{authorization}
\end{column}
\end{columns}
...
@Cookie@: @mediaWiki.user.id=m5MFt3bI4Lr1GcMvj4rPk4mwtwZqTWhx@; centralnotice_bannercount...
Connection: keep-alive
...
\end{lstlisting}
}

%TODO : transition + display code properly
\begin{frame}
...
%TODO note : mention exemple encryption algorithm ? 
\begin{frame}
...
%TODO : add a schema (??) and better centering
\begin{center}
\begin{tabular}{c|c}
\bfseries Symmetric & \bfseries Asymmetric \\
\hline
Fast & Slow \\
Needs a pre-shared key & No need for a pre-shared key \\
Owner(s) of the key & Owner of the private key \\
\end{tabular}
\end{center}
\note{
\begin{itemize}
...
\begin{frame}
%TODO : explain the motivation of hash functions
...
%------------------------------------------------
\begin{frame}
%TODO : explain WHY different passwords for each site
\frametitle{Password}
...
\begin{frame}
%TODO : ComicStrip is too small, too long, remove pics and explain orally
...
\begin{frame}
%TODO : change schema + remove CA explanation from notes
...
\begin{frame}
%TODO : move slide before the previous one and simplify schema
...
\begin{frame}
%TODO : change schema => client = web browser, server = web server
...
\begin{frame}
%TODO : smaller image, Explain N + ONCE
\frametitle{Signature challenge}
\begin{columns}
\begin{column}{0.6\linewidth}
\includeimage[width=\linewidth,height=0.7\textheight]{challenge-accepted}
\end{column}
\begin{column}{0.4\linewidth}
...
\begin{frame}
\frametitle{Kerberos}
\includeimage[width=3cm]{logo-kerberos}
\note{
...
\end{column}
\begin{column}[width=\linewidth]{0.3\linewidth}
\includeimage[width=0.8\linewidth]{logo-google}
\end{column}
\begin{column}[width=\linewidth]{0.3\linewidth}
\includeimage[width=0.8\linewidth]{logo-facebook}
\end{column}
\end{columns}
\note{
\begin{itemize}
...
%------------------------------------------------
\begin{frame}
%TODO : too small and add side channels (post, sms,...)
