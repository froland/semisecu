...
\bfseries
OWNS
\includeimage[width=0.85\linewidth]{key-icon}
\end{center}
\end{column}
\begin{column}{0.3\linewidth}
\begin{center}
...
\begin{frametransition}{What if an user loss its authentication means?}
~\\[1em]
...
\begin{itemize}
\item Deactivate a \emph{compromised} authentication means
\pause
\item Authenticate the user by an \emph{uncompromised} means
\pause
\item Give him a \emph{new} primary authentication means
\end{itemize}
\note{
When you implement authentication you have to take into account that
the user may lose its authentication means or it can get stolen.
\begin{itemize}
\item Deactivate a compromised authentication means
\item Authenticate the user by an uncompromised means
\item Give him a new primary authentication means
\end{itemize}
}
\end{frame}
...
\includeimage[width=0.7\linewidth]{security-question}
\pause
\item Another channel {\small (e-mail, post, sms)}
\end{itemize}
\end{Large}
%TODO add an image (such as SMS)
\note{
