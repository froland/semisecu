...
%TODO : expliquer en quoi les attaques sont complémentaires (remarque pour tout le chapitre)
...
\includeimage[width=\linewidth]{new-fbi-greendot-moneypak-virus}
\end{column}
...
% TODO make a video of the demo of the website
% (je vais la faire)
\end{exampleblock}
}
\end{frame}
...
% TODO ajouter widespread, high impact pour les attaques
...
\begin{frame}
% TODO parler plutot des frameworks que de JSP
\begin{embeddedcode}{bad}
\LstBad
\end{embeddedcode}
\badgoodsep
...
\begin{lstlisting}[style=beamer,basicstyle=\ttfamily\huge,text-align=center]
  http://host.com/page@;
...
\begin{itemize}
\item \small Public computer : Forgetting to log out...
\item Session fixation: In older application server, the session ID could be set using the URL.
You could send that URL to someone else.
\item Session guessing: Guess the next generated session ID.
\item XSS session hijacking - demo: The user's session can be obtained using XSS.
\end{itemize}
\end{exampleblock}
\hrule
...
\begin{itemize}
\item \small Example: Replace some object identifier in the request.
\item Impacts: Compromise the data that can be referenced.
\item Risks: Easy to exploit and detect. The impact is moderate.
\item Missing access verification: Often objects are retrieved by their ID, the developer can forget to check
whether the user is allowed to see that object.
\item An unvalidated hotel - demo: See an unvalidated hotel as a normal user.
\item Check permissions: Check whether the user has the permissions to access the object before
manipulating it.
...
\end{alertblock}
% TODO anecdote a ajouter? (je ne sais plus ce que je voulais dire par la...)
}
\end{frame}
...
\includeimage[width=0.7\linewidth]{webapp-forward}
\vspace{6mm}
\Lst
\note{
...
\includeimage[width=0.7\linewidth]{webapp-redirect}
\vspace{2mm}
\Lst
\note{
\begin{example}
...
%TODO : faire un plan des attaques qui vont suivre
% je dirais qu'il faut juste suivre un chemin logique comme pour la
% prevention, par exemple du serveur au client

\begin{frame}
\frametitle{Others attacks}
\begin{enumerate}
...
\end{frame}
% TODO reuse archi-webapplication
...
\end{itemize}
% TODO classify attacks (accordion presentation)
\note{
Bullet points.
...
\item Improper input validation
	\begin{itemize}
		...
		\item<5-6> Cross-Site Request Forgery (CSRF) %TODO ???
...
\item Education factor {\small (social engineering)}
	\begin{itemize}
		...
		\item<4-5> Phishing
		\item<5> (Brute Force) %car si les gens mettaient des mega password, brute force inutile
	\end{itemize}
\end{itemize}
\end{frame}
