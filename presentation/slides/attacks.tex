...
\item<1-> Improper input validation
	...
	\begin{itemize}
	\item<2-> Injection
	\item<3-> Cross-site scripting (XSS)
	\item<4-> Session Stealing via XSS
	\item<5-> Unvalidated Forwards and Redirects
	\end{itemize}
	}
\item<6-> Missing authorization checks
	\only<7-8>{
	\begin{itemize}
	\item<7-> Insecure Direct Object References
	\item<8-> Failure to Restrict URL Access
	\end{itemize}
	}
...
\item<10-> Other assumptions
	\only<11-14>{
	\begin{itemize}
	\item<11-> Insecure Cryptographic Storage
	\item<12-> Insufficient Transport Layer Protection
	\item<13-> Denial of Service
	\item<14-> Man in the Middle
	\end{itemize}
	}
\item<15-> Education factor {\small (social engineering)}
	\only<15-19>{
	\begin{itemize}
	\item<16-> Session stealing (cybercafe)
	\item<17-> Social engineering
	\item<18-> Phishing
	\item<19-> Brute Force
	\end{itemize}
	}
