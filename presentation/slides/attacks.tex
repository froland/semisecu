...
\end{column}
\begin{column}{0.6\linewidth}
%je ne trouve pas top cette image (krzysztof) C'est ce que j'ai trouvé de mieux...
...
\emphsidetext{easy to detect}
\includeimage[width=0.9\linewidth]{webapp-forward}
\Lst
\note{
...
\begin{itemize}
\item<1-> Improper input validation
  \only<2-5>{
  \begin{itemize}
  \item<2-5> Injection
  \item<3-5> Cross-site scripting (XSS)
  \item<4-5> Session Stealing via XSS
  \item<5-5> Unvalidated Forwards and Redirects
  \end{itemize}
}
\item<6-> Missing authorization checks
\only<7-8>{
  \begin{itemize}
  \item<7-8> Insecure Direct Object References
  \item<8-8> Failure to Restrict URL Access
  \end{itemize}
}
\item<9-> Misconfiguration
\item<10-> Other assumptions
\only<11-14>{
	\begin{itemize}
		\item<11-14> Insecure Cryptographic Storage
		\item<12-14> Insufficient Transport Layer Protection
		\item<13-14> Denial of Service
		\item<14-14> Man in the Middle
	\end{itemize}
}
\item<15-> Education factor {\small (social engineering)}
	\begin{itemize}
		\item<16-19> Session stealing (cybercafe)
		\item<17-19> Social engineering
		\item<18-19> Phishing
		\item<19-19> Brute Force
	\end{itemize}
\end{itemize}
\note{
Bullet point }
